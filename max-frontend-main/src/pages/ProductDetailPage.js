...
import { ShoppingBag, Heart, Star, ChevronLeft, Plus, Minus, Check, Truck, Shield, MessageCircle } from 'lucide-react';
...
export default function ProductDetailPage() {
  ...
  const handleEnquiry = () => {
    ...
    const whatsappUrl = `https://wa.me/918073121177?text=${encodeURIComponent(message)}`;
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 16, flexWrap: 'wrap' }}>
              ...
              <button onClick={handleEnquiry}
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '0 20px', height: 48, borderRadius: 'var(--radius-md)', border: '1px solid #25D366', background: 'rgba(37, 211, 102, 0.1)', color: '#25D366', fontWeight: 600, fontSize: 14, cursor: 'pointer', transition: 'var(--transition)', whiteSpace: 'nowrap' }}
                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(37, 211, 102, 0.2)'; e.currentTarget.style.boxShadow = '0 0 16px rgba(37, 211, 102, 0.15)'; }}
